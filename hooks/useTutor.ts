...
interface UseTutorOptions {
  ...
  onRateLimitUpdate?: (remaining: number) => void;
}
  ...
  const narrateText = useCallback(
    async (text: string): Promise<void> => {
      ...
      try {
        ...
        if ((e as Error).name !== 'AbortError') console.warn('Narration error:', e);
      } finally {
        setIsNarrating(false);
      }
    },
    ...
    async (question: string) => {
      ...
      try {
        ...
        if (typeof data.remaining === 'number') {
          options.onRateLimitUpdate?.(data.remaining);
        }

        const plan: ScenePlan = data.scenePlan;
        setCurrentPlan(plan);
        options.onScenePlanReady?.(plan);
        addMessage('assistant', plan.scenes.map((s) => s.narration).join(' '));
